...
const UserShipments = () => {
  ...
  const { list: userOrders, loading: ordersLoading, error: ordersError } = useSelector(
    (state) => state.orders
  );
  ...
  useEffect(() => {
    if (searchTerm) {
      ...
      setFilteredShipments(
        shipments.filter((s) =>
          [s.trackingNumber, s.status, s.assignedDriver?.name].some((f) =>
            f?.toLowerCase().includes(t)
          )
        )
      );
      setFilteredUserOrders(
        userOrders.filter((o) =>
          [o.orderCode, o.status, o.shop?.name].some((f) =>
            f?.toLowerCase().includes(t)
          )
        )
      );
  ...
  }, [shipments, userOrders, searchTerm]);
      ...
      <>
        ...
        <div className="shipments-container">
          <div className="no-shipments">
            <h4>Your Shipments</h4>
            <p>Please log in to view or create shipments</p>
            <button onClick={() => navigate('/home')} className="sign-in-button">
              Sign in
            </button>
          </div>
        </div>
  ...
  if (error) {
    return (
      <div className="shipments-error">
        <p>{error}</p>
        <button onClick={() => dispatch(fetchShipments(token))}>Retry</button>
      </div>
    );
  }
    ...
    <>
      ...
      {paymentModalOpen && selectedShipment && (
        <PaymentModal
          shipment={selectedShipment}
          onClose={() => setPaymentModalOpen(false)}
          refreshShipments={() => dispatch(fetchShipments(token))}
        />
      )}
      ...
      <div className="orders-section">
        ...
        {ordersLoading && userOrders.length === 0 ? (
          ...
          <div className="orders-error">
            <p>{ordersError}</p>
            <button onClick={() => dispatch(fetchUserOrders(token))}>Retry</button>
          </div>
        ) : filteredUserOrders.length === 0 ? (
          <div className="no-orders">
            <p>No orders found.</p>
          ...
          <div className="orders-list">
            {filteredUserOrders.map((o) => (
              <div key={o._id} className="order-card">
                <div className="order-header">
                  <h3>Order: {o.orderCode}</h3>
                  <span className="created-date">{formatDateTime(o.createdAt)}</span>
                </div>
                <div className="order-status">
                  <p><strong>Status:</strong> {o.status}</p>
                  <p><strong>Payment:</strong> {o.payment?.status}</p>
                </div>
              </div>
            ))}
          </div>
